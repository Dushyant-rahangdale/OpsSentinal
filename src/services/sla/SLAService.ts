...
export class SLAService {
    ...
    async generateDailySnapshot(slaId: string, targetDate: Date): Promise<SLASnapshot | null> {
        ...
        try {
            // 1. Fetch Definition
            const definition = await (prisma as any).sLADefinition.findUnique({ // eslint-disable-line @typescript-eslint/no-explicit-any
                where: { id: slaId }
            });
            ...
            if (definition.metricType === 'UPTIME' || definition.metricType === 'AVAILABILITY') {
                const metrics = await (prisma as any).metricRollup.findMany({ // eslint-disable-line @typescript-eslint/no-explicit-any
                    where: {
                ...
                const dateField = isMTTA ? 'acknowledgedAt' : 'resolvedAt';

                // Find incidents that were Acked/Resolved on this day
                // Using 'any' cast as fallback
                const incidents = await (prisma as any).incident.findMany({ // eslint-disable-line @typescript-eslint/no-explicit-any
                    where: {
                        serviceId: definition.serviceId,
                        [dateField]: {
                            gte: startOfDay,
                ...
                let totalCount = 0;
                let compliantCount = 0; // Requests faster than target
            ...
            const higherIsBetter = (definition.metricType === 'UPTIME' || definition.metricType === 'AVAILABILITY');
            ...
            // Idempotent upsert
            const snapshot = await (prisma as any).sLASnapshot.upsert({ // eslint-disable-line @typescript-eslint/no-explicit-any
                where: {
    ...
    async createDefaultSLA(serviceId: string): Promise<SLADefinition> {
        return await (prisma as any).sLADefinition.create({ // eslint-disable-line @typescript-eslint/no-explicit-any
            data: {
                serviceId,
