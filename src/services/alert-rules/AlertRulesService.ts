...
export class AlertRulesService {
    ...
    async createIncidentsForBreaches(evaluations: AlertEvaluation[]): Promise<string[]> {
        ...
        for (const evaluation of evaluations) {
            ...
            if (existing) {
                logger.info('[AlertRules] Incident already exists for rule', { ruleId: rule.id, incidentId: existing.id });
                continue;
            }
     ...
     */
    async runEvaluation(): Promise<{ evaluated: number; breached: number; incidentsCreated: string[] }> {
