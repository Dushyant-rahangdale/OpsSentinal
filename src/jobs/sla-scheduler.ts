...
export async function processSLASnapshots() {
    ...
    try {
        ...
        const definitions = await (prisma as any).sLADefinition.findMany({ // eslint-disable-line @typescript-eslint/no-explicit-any
            where: {
                activeFrom: { lte: endOfYesterday },
                OR: [
                    { activeTo: null },
                    { activeTo: { gte: startOfYesterday } }
                ]
            }
        });
        ...
        for (const def of definitions) {
            ...
            const existing = await (prisma as any).sLASnapshot.findUnique({ // eslint-disable-line @typescript-eslint/no-explicit-any
                where: {
