...
export async function runRetentionPolicy() {
    ...
    try {
        ...
        // Safe cast to 'any' to bypass missing client types until generation succeeds
        const configRecord = await (prisma as any).systemConfig.findUnique({ // eslint-disable-line @typescript-eslint/no-explicit-any
            where: { key: 'retention_policy' }
        });

        let policy = DEFAULTS;
        if (configRecord?.value && typeof configRecord.value === 'object' && !Array.isArray(configRecord.value)) {
            ...
            policy = {
                ...
                MINUTE_METRIC_DAYS: typeof custom.minuteMetrics === 'number' ? custom.minuteMetrics : DEFAULTS.MINUTE_METRIC_DAYS,
                HOURLY_METRIC_DAYS: typeof custom.hourlyMetrics === 'number' ? custom.hourlyMetrics : DEFAULTS.HOURLY_METRIC_DAYS,
            };
        }
        ...
        const deletedLogs = await (prisma as any).logEntry.deleteMany({ // eslint-disable-line @typescript-eslint/no-explicit-any
        ...
        const deletedMetrics = await (prisma as any).metricRollup.deleteMany({ // eslint-disable-line @typescript-eslint/no-explicit-any
            where: {
        ...
        logger.info('janitor.cleanup.metrics', { deleted: deletedMetrics.count, days: policy.MINUTE_METRIC_DAYS });
