...
export async function calculateSLAMetrics(filters: SLAMetricsFilter = {}): Promise<SLAMetrics> {
    ...
    const activeStatusWhere = filters.status ? { status: filters.status } : { status: { not: 'RESOLVED' as const } };
    ...
    const [
        ...
        prisma.incident.findMany({
            where: previousWhere,
            select: { id: true, createdAt: true, acknowledgedAt: true, resolvedAt: true, urgency: true, status: true }
        }),
    ...
    const [ackEvents, escalationEvents, reopenEvents, autoResolveEvents] = recentIncidentIds.length
        ? await Promise.all([
            prisma.incidentEvent.findMany({ where: { incidentId: { in: recentIncidentIds }, message: { contains: 'acknowledged', mode: 'insensitive' } }, select: { incidentId: true, createdAt: true } }),
            prisma.incidentEvent.findMany({ where: { incidentId: { in: recentIncidentIds }, message: { contains: 'escalated to', mode: 'insensitive' } }, select: { incidentId: true } }),
            prisma.incidentEvent.findMany({ where: { incidentId: { in: recentIncidentIds }, message: { contains: 'reopen', mode: 'insensitive' } }, select: { incidentId: true } }),
            prisma.incidentEvent.findMany({ where: { incidentId: { in: recentIncidentIds }, message: { contains: 'auto-resolved', mode: 'insensitive' } }, select: { incidentId: true } })
        ])
        : [[], [], [], []];
    ...
    const calculateSetMetrics = (incidents: typeof recentIncidents, eventsMap: Map<string, Date>) => {
        ...
        let resolveSum = 0, resolveCount = 0;
        let highUrg = 0;
    ...
    const prevAckSum = previousIncidents.reduce((sum, i) => sum + (i.acknowledgedAt ? i.acknowledgedAt.getTime() - i.createdAt.getTime() : 0), 0);
    const prevAckCount = previousIncidents.filter(i => i.acknowledgedAt).length;
    const prevResolveSum = previousIncidents.reduce((sum, i) => sum + (i.resolvedAt ? i.resolvedAt.getTime() - i.createdAt.getTime() : 0), 0);
    ...
    // Insights
    const insights = [];
    if (currentStats.count > prevStats.count) insights.push({ type: 'negative', text: `Incident volume up ${(currentStats.count - prevStats.count)} vs previous period` });
    else if (currentStats.count < prevStats.count) insights.push({ type: 'positive', text: `Incident volume down ${Math.abs(currentStats.count - prevStats.count)} vs previous period` });

    if (currentStats.mtta < prevStats.mtta && prevStats.mtta > 0) insights.push({ type: 'positive', text: `Response time improved by ${Math.round((1 - currentStats.mtta / prevStats.mtta) * 100)}%` });
    else if (currentStats.mtta > prevStats.mtta) insights.push({ type: 'negative', text: `Response time slower by ${Math.round((currentStats.mtta / prevStats.mtta - 1) * 100)}%` });

    // SLA Compliance Details
    let ackSlaMet = 0, resolveSlaMet = 0;
    const solvedIncidents = recentIncidents.filter(i => i.status === 'RESOLVED');
    const defaultAckTarget = 15, defaultResolveTarget = 120;
    ...
    const resolveCompliance = solvedIncidents.length ? (resolveSlaMet / solvedIncidents.length) * 100 : 100;
    ...
    // Service Map
    const serviceMap = new Map<string, { id: string, name: string, count: number, ackSum: number, ackCount: number, resolveSum: number, resolveCount: number, breaches: number }>();
    ...
    for (const incident of recentIncidents) {
        ...
        if (!serviceMap.has(incident.serviceId)) {
            serviceMap.set(incident.serviceId, {
                ...
                count: 0, ackSum: 0, ackCount: 0, resolveSum: 0, resolveCount: 0, breaches: 0
            });
        }
    ...
    const serviceMetrics = Array.from(serviceMap.values()).map(s => ({
        id: s.id,
        name: s.name,
        count: s.count,
    ...
    const autoResolveRate = solvedIncidents.length ? (autoResolvedCount / solvedIncidents.length) * 100 : 0;
    ...
    // Status Mix
    const statusOrder: typeof allowedStatus[number][] = ['OPEN', 'ACKNOWLEDGED', 'SNOOZED', 'SUPPRESSED', 'RESOLVED'];
    ...
    // Assignee Load
    const assigneeIds = assigneeCounts.map(e => e.assigneeId).filter((id): id is string => Boolean(id));
    const usersById = assigneeIds.length ? await prisma.user.findMany({ where: { id: { in: assigneeIds } }, select: { id: true, name: true, email: true } }) : [];
    ...
    return {
        ...
        mtti: null, mttk: null, mttaP50: null, mttaP95: null, mttrP50: null, mttrP95: null, mtbfMs,
        ...
        // Golden Signals
        avgLatencyP99: null, errorRate: null, totalRequests: 0, saturation: null,
        ...
        eventsPerIncident: totalRecent > 0 ? (ackEvents.length + escalationEvents.length + reopenEvents.length + autoResolveEvents.length) / totalRecent : 0,
        heatmapData
    };
}
...
export async function checkIncidentSLA(incidentId: string): Promise<any> {
    ...
    let ackBreached = false, ackTimeRemaining: number | null = null;
    ...
    let resolveBreached = false, resolveTimeRemaining: number | null = null;
    ...
    return {
        ackSLA: { breached: ackBreached, timeRemaining: ackTimeRemaining, targetMinutes: targetAckMinutes },
        resolveSLA: { breached: resolveBreached, timeRemaining: resolveTimeRemaining, targetMinutes: targetResolveMinutes }
    };
}
