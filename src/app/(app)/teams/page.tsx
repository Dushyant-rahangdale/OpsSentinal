                ...
                {teams.length === 0 ? (
                    ...
                    return (
                        <div key={team.id} className="glass-panel" style={{ padding: '1.5rem', background: 'white' }}>
                            ...
                            <form action={updateTeamWithId} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '1rem', alignItems: 'end', marginBottom: '1.5rem' }}>
                                <div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem' }}>
                                                    ...
                                                    <div key={member.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', border: '1px solid var(--border)', borderRadius: '10px' }}>
                                                        ...
                                                        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                                            <form action={updateMemberRole} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                                                <select name="role" defaultValue={member.role} disabled={isSoleOwner} style={{ padding: '0.4rem 0.6rem', border: '1px solid var(--border)', borderRadius: '6px' }}>
                                                                    <option value="OWNER">Owner</option>
                                                                    <option value="ADMIN">Admin</option>
                                                                    <option value="MEMBER">Member</option>
                                                                </select>
                                                                <button type="submit" className="glass-button" disabled={isSoleOwner}>Save</button>
                                                            </form>
                                                            <form action={removeMember}>
                                                                <button type="submit" disabled={isSoleOwner} title={isSoleOwner ? 'Reassign owner before removing.' : 'Remove member'} style={{ background: isSoleOwner ? '#fca5a5' : '#fee2e2', color: 'var(--danger)', border: 'none', padding: '0.4rem 0.75rem', borderRadius: '999px', cursor: isSoleOwner ? 'not-allowed' : 'pointer', fontWeight: '600', fontSize: '0.75rem' }}>
                                                                    Remove
                                                                </button>
                                                            </form>
                                                            {isSoleOwner ? (
                                ...
                                <div>
                                    ...
                                    <form action={addMemberWithId} style={{ display: 'grid', gap: '0.75rem' }}>
                                        ...
                                        <select name="role" defaultValue="MEMBER" style={{ padding: '0.6rem', border: '1px solid var(--border)', borderRadius: '6px' }}>
                                            <option value="OWNER">Owner</option>
                                            <option value="ADMIN">Admin</option>
                                            <option value="MEMBER">Member</option>
                                        </select>
