...
export default function SsoSettingsForm({ initialConfig, callbackUrl, hasEncryptionKey }: Props) {
    const [state, formAction] = useActionState<State, FormData>(saveOidcConfig, { error: null, success: false });
    ...
    return (
        <form action={formAction} className="settings-form-stack">
            ...
            >
                <label className="settings-toggle">
                    <input
                        type="checkbox"
                        name="enabled"
                        defaultChecked={initialConfig?.enabled ?? false}
                    />
            ...
            <SettingRow
                ...
                helpText={initialConfig?.hasClientSecret ? 'Leave blank to keep the current secret.' : 'Required for new configuration.'}
            >
                <input
                    type="password"
                    name="clientSecret"
