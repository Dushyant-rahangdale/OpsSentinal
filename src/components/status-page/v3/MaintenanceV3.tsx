...
/** Maintenance cards straight from the V3 `maintenance` section ΓÇö no announcement-type guessing. */
export default function MaintenanceV3({
  ...
  return (
    <section
      className="status-v3-maintenance status-panel"
        ...
        {maintenance.map(item => (
          ...
          >
            ...
            <span className="status-muted" suppressHydrationWarning>
              ...
              {item.endAt
                ? ` ΓåÆ ${formatDateTime(item.endAt, timeZone, { format: 'short', hour12: true })}`
                : ''}
            </span>
