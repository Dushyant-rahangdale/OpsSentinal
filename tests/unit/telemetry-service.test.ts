...
describe('TelemetryServiceV2', () => {
  const prismaMock = prisma as unknown as {
    metricRollup: { createMany: ReturnType<typeof vi.fn> };
    logEntry: { createMany: ReturnType<typeof vi.fn> };
  };
  ...
  it('should aggregate metrics with same tags', () => {
    ...
    return telemetryState.flushMetrics().then(() => {
      expect(prismaMock.metricRollup.createMany).toHaveBeenCalledTimes(1);
      const callArgs = vi.mocked(prismaMock.metricRollup.createMany).mock.calls[0][0] as {
        data:
          | Array<{ count: number; sum: number; min: number; max: number }>
  ...
  it('should flush logs when batch size is reached', async () => {
    ...
    expect(prismaMock.logEntry.createMany).toHaveBeenCalled();
  });
});
