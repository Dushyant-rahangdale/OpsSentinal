...
describe('analytics-metrics utilities', () => {
    ...
    it('builds status age averages', () => {
        ...
        const incidents = [
            ...
            { status: 'RESOLVED', createdAt: new Date('2025-01-01T00:00:00Z'), resolvedAt: new Date('2025-01-01T06:00:00Z') }
        ];
    ...
    it('builds on-call load totals', () => {
        const shifts = [
            { userId: 'u1', start: new Date('2025-01-01T00:00:00Z'), end: new Date('2025-01-01T06:00:00Z') },
            { userId: 'u2', start: new Date('2025-01-01T06:00:00Z'), end: new Date('2025-01-01T12:00:00Z') }
        ];
        const incidents = [
            { createdAt: new Date('2025-01-01T01:00:00Z') },
            { createdAt: new Date('2025-01-01T07:00:00Z') }
        ];
        const userNameMap = new Map<string, string>([['u1', 'User One'], ['u2', 'User Two']]);
        const load = buildOnCallLoad(shifts, incidents, new Date('2025-01-01T00:00:00Z'), new Date('2025-01-01T12:00:00Z'), userNameMap);
    ...
    it('builds service SLA compliance table', () => {
        ...
        const targets = new Map<string, { ackMinutes: number; resolveMinutes: number }>([['s1', { ackMinutes: 15, resolveMinutes: 90 }]]);
