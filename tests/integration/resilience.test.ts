...
describeIfRealDB('Database Resilience Integration Tests', { timeout: 30000 }, () => {
    ...
    describe('On-Call Schedule Resilience', () => {
        it('should resolve to override user during override window', async () => {
            ...
            const service = await createTestService('Critical Service', null, { escalationPolicyId: policy.id });
            ...
            const notifications = await testPrisma.notification.findMany({ where: { incidentId: incident.id } });
    ...
    describe('Multi-Step Escalation Chain', () => {
        it('should advance through multiple steps correctly', async () => {
            ...
            const service = await createTestService('Tiered Service', null, { escalationPolicyId: policy.id });
            ...
            const allNotifications = await testPrisma.notification.findMany({ where: { incidentId: incident.id } });
    ...
    describe('Concurrency and Locking', () => {
        it('should prevent duplicate escalations via lock', async () => {
            ...
            const service = await createTestService('Lock Service', null, { escalationPolicyId: policy.id });
            ...
            const failCount = [res1, res2].filter(r => !r.escalated && r.reason === 'Escalation already in progress').length;
    ...
    describe('Graceful Recovery', () => {
        it('should skip to next step if target is invalid', async () => {
            ...
            const service = await createTestService('Recovery Service', null, { escalationPolicyId: policy.id });
